...
Interactive playground for `vuqs` and `@vuqs/store`. Every page syncs state to the URL — edit a
control and watch the inspector, or edit the URL and watch the UI react. State survives a reload
...
| `/store`   | `@vuqs/store`: `selected` (URL) vs async-loaded `defaults` vs derived `effective` |
| `/context` | Context-aware `preserve` / reset / `only` validity across tabs, in one navigation |

## How it resolves the packages

`vite.config.ts` aliases `vuqs`, `vuqs/adapters/vue-router`, and `@vuqs/store` to their `src/`
