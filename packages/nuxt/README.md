...
The composables, the `codecs` namespace, and the modules are auto-imported, and
...
- Composables: `useQueryState`, `useQueryStates`, `useQueryAdapter`,
  `provideQueryAdapter`, `queryParam`, `defineQueryModule`,
  `createSerializer`
- Codecs: `codecs`, `createCodec`. `codecs` is a single namespace object, so it is
  one auto-imported name (`codecs.string`, `codecs.integer`, and so on), not one
  per codec
...
| `debug`       | `boolean \| 'force' \| { client?, server? }` | `false` | Enable console diagnostics. The shorthand targets the browser; server logging is separately opt-in and request-scoped. |
