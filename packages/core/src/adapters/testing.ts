...
export interface UrlUpdateEvent {
  ...
  /** The resolved navigation options for this navigation. */
  options: NavigateOptions
}

/**
 * A callback invoked on each flushed navigation. Wire it to a spy (`vi.fn()`) to
 * assert on URL updates without mocking the router.
 */
...
export interface TestingAdapterOptions {
  ...
  /** Invoked on each flushed navigation. Coalesced transactions produce one callback. */
