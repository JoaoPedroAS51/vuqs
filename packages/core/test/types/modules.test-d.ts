...
describe('module composition', () => {
  ...
  it('types atomic transactions and schema-projected starts', () => {
    const withTransactions = (core: QueryCore<typeof schema>): { stop: () => void } => {
      ...
      // @ts-expect-error transaction values follow the schema value types
      core.query.transact({ mode: 'patch', values: { category: 'memory' } })
      // @ts-expect-error default policy is a closed union
      core.query.transact({ mode: 'patch', values: {}, defaultPolicy: 'preserve' })
