...
describe('useQueryStates', () => {
  ...
  describe('patch', () => {
    ...
    it('rejects keys not in the schema without applying a partial write', async () => {
      ...
      expect(() => patch({ q: 'x', nope: 'y' })).toThrow('cannot write unknown query-state key "nope"')
      await flush()

      expect(query.value).toEqual({ tab: 'a' })
      expect(values.q).toBeUndefined()

      // an unrelated URL change must not disturb this binding's projection
      query.value = { tab: 'b' }
      await flush()
  ...
  describe('clearOnDefault', () => {
    ...
    it('composable options override keepOnDefault', async () => {
      const schema = {
        page: queryParam('page', codecs.integer).withDefault(1).keepOnDefault(),
      }
      const { query, run } = setup()
