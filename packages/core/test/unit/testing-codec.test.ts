...
describe('isCodecBijective', () => {
  ...
  it('throws when serialize(input) does not produce the expected serialized value', () => {
    // serialize(99) = '99', but the expected serialized value is '42'. This fails
    // before the round-trip checks.
  ...
  it('throws when parse(serialized) does not match input, independent of the round-trip checks', () => {
    // An asymmetric `eq` can pass both round-trip checks even though `eq` must be
    // an equivalence relation. The direct input comparison detects it.
