...
This is the building block for "next page" links that retain the current filters:
...
are affected: the serializer never injects phantom defaults for params you did not
touch.
...
exported for advanced use. To read values out of a query rather than build one,
pair the schema with [`parseQueryStates`](/api/serializer#pure-functions). See the
[API reference](/api/serializer#pure-functions) for the full set.
